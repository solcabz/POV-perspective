...
const playertime =
  '.CoreText-sc-1txzju1-0.ffZeRf[data-a-target="player-seekbar-current-time"]'; //.innertext of th
...
function looper() {
  ...
  setTimeout(() => {
    if (document.querySelector(slashvideos) != null) {
      //checks if any of the selectors are on page
      if (
        document.getElementById("finderwrap") != null &&
        document.querySelector("#finderwrap").classList[0] !=
        inputUrl.substring(inputUrl.lastIndexOf("/") + 1)
      ) {
        document.getElementById("finderwrap").remove();
      } //checks if box exists and if the box class does not match url it removes the box
      if (document.getElementById("finderwrap") === null) {
        urlchecker();
      } //if the box does not exist check url and probably create one
      else {
        looper();
      }
    } else {
...
const apireturnexpress = async (input) => {
  ...
  if (a.user.videos.edges.length === 0) {
    return;
  } //if user exists but has no videos just exit
...
function userquery(input, ninput) {
...
const apireturnv = async (input) => {
  //video information api call
  const a = await apifetch(input);
  console.log(a);
  if (a.video === null) {
    document.getElementById("infodiv").innerText =
      "API didn't return vod info.";
    document.getElementById("infodiv").style.color = "#D68029";
  } else if (
    a.video.broadcastType != "ARCHIVE" &&
    a.video.broadcastType != null
  ) {
    document.getElementById("infodiv").innerText =
      "This is not a vod. Looking Glass will not function properly.";
    document.getElementById("infodiv").style.color = "#D68029";
    ...
    document
      .getElementById("finderwrap")
      .setAttribute("data-time", Date.parse(a.video.createdAt) / 1000); //start time of vod
  }
};

const apireturnu = async (input, name) => {
  //user input to get list of videos api call
  const a = await apifetch(input);
  if (a.user == null) {
    document.getElementById("infodiv").innerText =
      "Invalid Twitch Name. Try Again.";
    return;
  }
  if (a.user.videos.edges.length === 0) {
...
function boxcreate(qselector, type) {
  ...
  bot.addEventListener("contextmenu", function(event) {
    //this just prevents context menu from popping up if you misRclick around bubbles
    event.preventDefault();
  });

  var createind = document
    .getElementById("topd")
    .appendChild(document.createElement("div"));
  createind.setAttribute("id", "ind");

  var createp = document
    .getElementById("ind")
    .appendChild(document.createElement("input"));
  createp.setAttribute("id", "targetid");
  createp.setAttribute("placeholder", "Channel Name");

  var createinfod = document
    .getElementById("topd")
    .appendChild(document.createElement("div"));
  createinfod.setAttribute("id", "infodiv");
  createinfod.innerText =
    "Enter name to search vods. Ctrl+Enter to instantly open links.";
...
function start() {
  ...
  if (
    document.getElementById("finderwrap").getAttribute("data-type") === "vod"
  ) {
    document
      .getElementById("finderwrap")
      .setAttribute(
        "data-offset",
        cleanTime(document.body.querySelector(playertime).innerText)
      ); //get seconds back
...
function getinput() {
  ...
  console.log(iname);
  if (iname === "") {
    document.getElementById("infodiv").innerText = "Empty input. Enter name.";
    return;
  }
...
function arrayvods(varray, name) {
  ...
  var vodend = [];
  const time = parseInt(
    document.getElementById("finderwrap").getAttribute("data-time")
  ); //these are here just for readability
  ...
  for (var i = 0; i < varray.length; i++) {
    //gets start and end times of inputed user's last 100 vods
  ...
  if (stime < vodstart[vodstart.length - 1]) {
    ...
    document.getElementById(
      "infodiv"
    ).innerText = `Searched ${varray.length} vod(s) and this is older than all of them. Rclick to remove bubble.`;
    cl(link, color, name);
    return;
  } else {
    for (var i = 0; i < vodstart.length; i++) {
      if (stime > vodstart[i] && stime < vodend[i]) {
        ...
        const link = "https://twitch.tv/videos/"
          .concat(varray[i].node.id.concat("?t="))
          .concat(ts);
        var color = "green";
        cl(link, color, name);
        document.getElementById("infodiv").innerText = `${
          varray.length
        } vod(s) and this was found ${
          i + 1
        } vod(s) ago! Rclick bubble for details.`;
        return;
      }
    }
    var color = "red";
    ...
    document.getElementById(
      "infodiv"
    ).innerText = `${varray.length} vod(s) and this timestamp wasn't found in any of them. try to adjust timestamp, Rclick to remove bubble.`;
  }
}

function secondsCalc(d) {
  //input seconds get out 1h2m3s
...
function cleanTime(time) {
  //input example 00:46:12 takes raw vod time and cleans it to 46m12s
...
function cl(link, color, name) {
  ...
  if (color === "green") {
    (ce.style.background = "#7AC06F") && (ce.style.cursor = "pointer");
  }
  if (color === "red") {
    (ce.style.background = "#C55C62") && (ce.style.cursor = "default");
  }
  if (color === "gray") {
    (ce.style.background = "#505050") && (ce.style.cursor = "default");
  }
